...
class mge_fit_1d(object):
    ...
    def __init__(self, x, y, negative=0, ngauss=12,
                 rbounds=None, outer_slope=4, quiet=False, plot=False):
        """
        ...
        if rbounds is None:
            lrbounds = np.log10([min(x), max(x)/np.sqrt(outer_slope)])
        else:
            lrbounds = np.log10(rbounds)

        logsigma = _linspace_open(*lrbounds, num=ngauss)
        ...
        w = self.weights != 0
        self.weights, self.gauss, logSigma = self.weights[w], self.gauss[:, w], mp.params[w]
        j = np.argsort(logSigma)
        self.sol = np.vstack([self.weights[j], 10**logSigma[j]])
        ...
        if not quiet:
            ...
            print('Nonzero Gaussians: ', logSigma.size)
            print(' Unused Gaussians: ', ngauss - logSigma.size)
            ...
            for sol in self.sol.T:
                print('%13.6g %13.6g' % (sol[0], sol[1]))
            print('############################################')

        if plot:
            self._plot(mp.params)
...
#----------------------------------------------------------------------------
    ...
    def _fitfunc(self, logsigma, fjac=None):
        ...
        self.yfit = self.gauss.dot(self.weights)
