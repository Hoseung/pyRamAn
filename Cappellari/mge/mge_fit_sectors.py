...
OPTIONAL INPUT KEYWORDS:
      /BULGE_DISK - Set this keyword to perform a non-parametric bulge/disk
      ...
      /PRINT - Set this keyword to print the best-fitting MGE profiles
...
MODIFICATION HISTORY:
      ...
      V5.0.3: Make sure qbounds is a Numpy array. Include absdev in the class attributes.
        Nicely format printed solution. MC, Atlantic Ocean, 28 March 2015
      V5.0.4: Fully broadcast _fitfunc. MC, Atlantic Ocean, 6 June 2015
      V5.0.5: Fixed plotting issue when combining profiles from multiple images.
        Thanks to Arianna Picotti (MPIA) for the bug report with examples.
        Only plot profiles for the best-fitting MGE. MC, Oxford, 18 June 2015
...
from Cappellari.mge.cap_mpfit import mpfit

#----------------------------------------------------------------------------

def _linspace_open(a, b, num=50):
        x, dx = np.linspace(a, b, num, endpoint=False, retstep=True)
        return x + dx/2.
...
class mge_fit_sectors(object):
    ...
    def __init__(self, radius, angle, counts, eps,
                 ...
                 outer_slope=4, bulge_disk=False, sol=0, plot=False, debug=False):
        """
        ...
        if not (radius.size == angle.size == counts.size):
        ...
        self.scale = scale
        self.quiet = quiet
        self.plot = True if debug else False
        ...
        if linear:
            ...
            if qbounds.size == 4:
                raise ValueError('Multiple qbounds not supported with LINEAR keyword')
            if ngauss is None:
                ngauss = 100**2
            elif ngauss < 10**2:
        ...
        if not self.quiet:
            ...
            for sol in self.sol.T:
                print('%13.6g %13.6g %13.6f' % (sol[0], sol[1], sol[2]))
            print('++++++++++++++++++++++++++++++++++++++++++++')
...
#----------------------------------------------------------------------------
    ...
    def _fitfunc(self, pars, fjac=None):
        ...
        if self.negative:   # Solution by LAPACK linear least-squares
            self.weights = linalg.lstsq(A, b, cond=0.1)[0]
        else:               # Solution by NNLS
            self.weights = optimize.nnls(A, b)[0]

        self.yfit = self.gauss.dot(self.weights)   # Evaluate predictions by matrix multiplications
...
#----------------------------------------------------------------------------
    ...
    def _print(self, myfunct, logsigma, it, fnorm,
        ...
        """
        print('Iteration: ', it, '  chi2: %.4g' % fnorm, ' Nonzero:', np.sum(self.weights != 0))
