subroutine count_part(ndm_actual, nstar_actual, nsink_actual, repository, xmin, xmax, ymin, ymax, zmin, zmax, star)
  !--------------------------------------------------------------------------
...
!  input format with constant string is not fortran standard. works only with ifort.
  ...
  read(10,'(13x,E23.15)')unit_l
  read(10,'(13x,E23.15)')unit_d
  read(10,'(13x,E23.15)')unit_t
  read(10,*)
...
!       x(1:npart2,0)=m/boxlen
	...
	! ignore metal
     ...
     do i=1,npart2
          ...
          ok_dm=ok_part.and.(age(i).eq.0.0d0).and.(id(i)>0)
          ok_star=ok_part.and.(age(i).ne.0.0d0).and.(id(i)>0)
          ok_sink=ok_part.and.(age(i).eq.0.0d0).and.(id(i)<0)
  ...
  subroutine part2map(ndm_actual, nstar_actual, nsink_actual, repository, xmin, xmax, ymin, ymax, zmin, zmax, star)
  ...
  allocate(sink_arr(1:nsink_actual,8)) ! id,x,y,z,vx,vy,vz,mass
...
!        ok_sink=ok_part.and.(age(i).eq.0.0d0).and.(id(i)<0)
	...
	if(ok_part.and.(age(i).eq.0.0d0).and.(id(i)>0))then
	   ...
	   star_arr(i_star,7) = m(i,2)
	   star_arr(i_star,8) = id(i,2)
	   star_arr(i_star,9) = age(i,2)
	   star_arr(i_star,10) = metal(i,2)
	   i_star = i_star + 1
	elseif(ok_part.and.(age(i).eq.0.0d0).and.(id(i)<0))then
	   ...
	   sink_arr(i_sink,7) = m(i,2)
	   sink_arr(i_sink,8) = id(i,2)
           i_sink = i_sink + 1
        endif
     enddo
     deallocate(x,v,m,id)
