...
# it's faster to use ~i_dm than to generate another index array.
    ...
    def load_fortran(self,
        ...
        if verbose: print(self.ndm, self.nstar, self.nsink, self.ntracer)
        if npart_tot == 0:
            return
        """
        ...
        if 'sink' in self.pt:
            dtype_sink = dtype_dm
            ind_sink = np.where(isnt_star * (part_int < 0)*(part_float[:,6] > 0))[0]
        ...
        if 'tracer' in self.pt:
            dtype_tracer = {'id': (('<i8', 1), 0),
                            ...
                            'vel': (('<f8', (3,)), 32),
                             ...
                             'vy': (('<f8', 1), 40),
                             'vz': (('<f8', 1), 48)}
            ind_tracer = np.where(isnt_star * (part_int < 0)*(part_float[:,6] == 0))[0]
