...
OUTPUTS:
      ...
      Xpeak = First index (row) in Img, of the pixel containing the galaxy center.
          ...
          pixels within a 40x40 pixels box centered on the galaxy weighted average center.
          IMPORTANT: The brightest galaxy pixel is the element Img[Xpeak, Ypeak].
            In the plot produced by find_galaxy, the brightest pixel is at
            coordinates (Ypeak, Xpeak), with the two coordinates swapped!
      Ypeak = Second index (column) in Img, of the pixel containing the galaxy center.
...
OPTIONAL INPUT KEYWORDS:
      ...
      MASK_SHADE - shade on top of original image
...
class find_galaxy(object):
    ...
    def __init__(self, img, fraction=0.1, plot=False, mask_shade=True,
                 quiet=False, nblob=1, level=None, binning=5):
        """
        ...
        if plot:
            ...
            ax.plot(xc - mjr*np.sin(ang), yc + mjr*np.cos(ang), 'g--',
                    xc + mjr*np.cos(ang), yc + mjr*np.sin(ang), 'g-', linewidth=1)
