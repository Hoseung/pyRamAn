...
def load_data(f, tothal):
    import numpy as np
#    from load.utils import read_fortran
    from load.ctuils import read_fortran
    ...
    for i in range(tothal):
        ...
        data['hhost'][i] = read_fortran(f, np.dtype('i4'), 5)
        data['m'][i] = read_fortran(f, np.dtype('f4'), 1)
        data['p'][i] = read_fortran(f, np.dtype('f4'), 3)
        data['v'][i] = read_fortran(f, np.dtype('f4'), 3)
        data['ang'][i] = read_fortran(f, np.dtype('f4'), 3)
