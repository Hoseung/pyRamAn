...
if __name__ == "__main__":
    ...
    if is_gal:
        Mcut = 1e10
        if test:
            basedir = wdir + "test_fine_direct_prgs_gal/"
        else:
            basedir = wdir + "MJ_fine_direct_prgs_gal/"
    if not is_gal:
        Mcut = 5e10
        if test:
            basedir = wdir + "test_direct_prgs_gal/"
        else:
            basedir = wdir + "MJ_direct_prgs_hal/"
    ...
    for i, (fid,fidx) in enumerate(zip(final_ids, final_idxs)):
        good =refined_tree.refined_tree(tt, fidx,
                                  f_dist_sum = 0.66,
                                  r_fi = 0.5,
                                  step_early_enough = 120,
                                  m_small_enough = 1e7,
                                  too_short_ref = 2,
                                  threshold_score=2.0,
                                  l_tree_max=100,
                                  l_too_short_close=25,
                                  do_plot=True,
                                  out_dir=basedir)
        if good:
    ...
    pickle.dump(good_idxs, open(basedir+"final_ids_good_massive.pickle", "wb"))
