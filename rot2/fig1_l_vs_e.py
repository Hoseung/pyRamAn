...
def lambda_vs_e_raw(final_data, mass_bin=[0,1e20], out_base='./'):
    """
    Parameters
    ----------
...
def lambda_vs_e(final_data, istep=0, mass_bin=None, out_base='./'):
    """
    Parameters
    ----------
        final_data : allresults at a nout. (allresults[0])
    ...
    do_plot(x,y, -10*np.arange(6).reshape(2,3),
            ...
            d_alpha=1.0,
            levels=None,#np.linspace(0.02, 1.0, 19),
            fname_vs_e = out_base+ "figs/lambda_vs_e__")
    ...
    for gg in final_data:
        try:
            eps.append(gg.data[0][0][istep].mge_result_list[0]["eps"])
            lambda_e.append(gg.main_arr["lambda_r"][istep])
            mass.append(gg.main_arr["mstar"][istep])
        except:
            pass
    #lambda_e = lambda_e[:len(eps)]
    ...
    twocolors=['#4c72b0', '#c44e52']
    #red = '#c44e52'
    all_plot_modules.do_plot(x,y, atlas,
            do_scatter=False,
            contour_label=False,
            surf = False,
            img_scale = 2.0,
    ...
    all_plot_modules.do_plot(x,y, None,
            do_scatter=False,
            contour_label=False,
            surf = False,
            img_scale = 2.0,
