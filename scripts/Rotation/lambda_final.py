...
def not_all_galaxies_have_hosthalo(allhal, allgal):
	...
	for i, this_hal in enumerate(allhal.data['id']):
		...
		if len(ind_idlist) > 0:
			...
			# If i-th galaxy has no host halo, what should I do..?
			# mk_gal takes DM nearby particles instead of using IDlist
			# when the length of IDlist is shorter than 64.
			# So an empty IDlist will invoke lambda_final.mk_gal() function to 
			# take
			newlist[i] = np.array([])
...
def main(wdir='./',
    ...
    if not read_halo_list:
        ...
        pickle.dump(prg_only_tree, open(wdir + 'prg_only_tree.pcikle', 'wb'))
