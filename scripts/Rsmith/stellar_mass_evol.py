...
# Static memory automatically overwritten during iteration.
# But I am not sure whether the memory chunks are correctly freed or not.
...
ncore = 4
# I definitely see the speed-up in shared_Array.py script,
# But I don't see in this script. 
# Why....? 
# memory is being copied rather than shared..? 
fixed_position = True
work_dir = './'
nout_ini = 37
# 27 : z=4
...
#    hh = tree.hmhalo.Hmhalo(fn)
    ...
    h_ind = match.match_list_ind(data['id'], halo_ok[:,inout])
    h = hmo.Halo()
    h.derive_from(hh, h_ind)
#    galaxy_catalog(work_dir, nout, info, h)
        
#def galaxy_catalog(wdir, nout, info, h):
    import utils.sampling as smp
