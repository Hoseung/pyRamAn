...
def plot_violin(mpgs, mstar_limit=1e10, suffix="", wdir='./'):
    ...
    dlm_all = remove_nan(np.array(dlm_all))
    dlt_all = remove_nan(np.array(dlt_all))
...
nout_ini = 57
cdir = 'easy_final/'
suffix = ["10002", "04466", "29172", "17891", "29176"][1]
wdir = './' + suffix + '/'
dir_out = wdir 
# "10002"
...
# In[10]:
# put all catalogs together. 
# Should I correct missing values and so smoothing here? 
mpgs = mma.compile_mpgs(alltrees,
                        idx_all,
                        wdir = wdir,
                        cdir = cdir,
                        nout_ini = nout_ini,
...
#Merger epochs
mma.find_merger_epochs(alltrees, 
                        idx_all, 
                        mpgs, 
                        nout_ini=nout_ini, 
                        dist_gal_scale=4,
...
#mma.measure_delta(mpgs, dt_before=7, dt_after=10, nout_ini=nout_ini)
