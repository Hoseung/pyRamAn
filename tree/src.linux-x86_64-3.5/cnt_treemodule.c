 ...
 * Generation date: Mon May  8 01:49:44 2017
 * $Revision:$
 * $Date:$
...
n_halos_all,flist_index,slist_index,nsteps = count_tree(fn,big_run)\n\nWrapper for ``count_tree``.\
\n\nParameters\n----------\n"
...
/*decl*/
  ...
  static char *capi_kwlist[] = {"fn","big_run",NULL};
...
f2py_start_clock();
  ...
  if (!PyArg_ParseTupleAndKeywords(capi_args,capi_keywds,\
    "OO:cnt_tree.count_tree",\
    capi_kwlist,&fn_capi,&big_run_capi))
    return NULL;
/*frompyobj*/
...
/*end of pyobjfrom*/
    ...
    capi_buildvalue = Py_BuildValue("iiii",n_halos_all,flist_index,slist_index,nsteps);
/*closepyobjfrom*/
...
fatherid,fatheridx,sonid,fathermass,i_arr,f_arr,aexp_arr,omega_t_arr,age_univ_arr = load_tree(fn,n_halos_all,n_all_fathers,n_all_sons,big_run,nsteps)\n\nWrapper for ``load_tree``.\
\n\nParameters\n----------\n"
"fn : input string(len=256)\n"
"n_halos_all : input int\n"
"n_all_fathers : input int\n"
"n_all_sons : input int\n"
"big_run : input int\n"
"nsteps : input int\n"
"\nReturns\n-------\n"
"fatherid : rank-1 array('i') with bounds (n_all_fathers)\n"
"fatheridx : rank-1 array('i') with bounds (n_all_fathers)\n"
"sonid : rank-1 array('i') with bounds (n_all_sons)\n"
"fathermass : rank-1 array('f') with bounds (n_all_fathers)\n"
"i_arr : rank-2 array('i') with bounds (n_halos_all,14)\n"
"f_arr : rank-2 array('f') with bounds (n_halos_all,25)\n"
"aexp_arr : rank-1 array('f') with bounds (nsteps)\n"
"omega_t_arr : rank-1 array('f') with bounds (nsteps)\n"
"age_univ_arr : rank-1 array('f') with bounds (nsteps)";
/* extern void F_FUNC_US(load_tree,LOAD_TREE)(string,int*,int*,int*,float*,int*,float*,float*,float*,float*,int*,int*,int*,int*,int*,size_t); */
...
/*decl*/
  ...
  static char *capi_kwlist[] = {"fn","n_halos_all","n_all_fathers","n_all_sons","big_run","nsteps",NULL};
...
#endif
  if (!PyArg_ParseTupleAndKeywords(capi_args,capi_keywds,\
    "OOOOOO:cnt_tree.load_tree",\
    capi_kwlist,&fn_capi,&n_halos_all_capi,&n_all_fathers_capi,&n_all_sons_capi,&big_run_capi,&nsteps_capi))
    return NULL;
/*frompyobj*/
  /* Processing variable n_halos_all */
    f2py_success = int_from_pyobj(&n_halos_all,n_halos_all_capi,"cnt_tree.load_tree() 2nd argument (n_halos_all) can't be converted to int");
  if (f2py_success) {
  /* Processing variable big_run */
    big_run = (int)PyObject_IsTrue(big_run_capi);
    f2py_success = 1;
  ...
  /* Processing variable n_all_fathers */
    f2py_success = int_from_pyobj(&n_all_fathers,n_all_fathers_capi,"cnt_tree.load_tree() 3rd argument (n_all_fathers) can't be converted to int");
  if (f2py_success) {
  /* Processing variable fn */
  slen(fn) = 256;
  ...
  if (f2py_success) {
  /* Processing variable nsteps */
    f2py_success = int_from_pyobj(&nsteps,nsteps_capi,"cnt_tree.load_tree() 6th argument (nsteps) can't be converted to int");
  if (f2py_success) {
  /* Processing variable n_all_sons */
    f2py_success = int_from_pyobj(&n_all_sons,n_all_sons_capi,"cnt_tree.load_tree() 4th argument (n_all_sons) can't be converted to int");
  if (f2py_success) {
  /* Processing variable age_univ_arr */
  age_univ_arr_Dims[0]=nsteps;
  capi_age_univ_arr_intent |= F2PY_INTENT_OUT|F2PY_INTENT_HIDE;
  capi_age_univ_arr_tmp = array_from_pyobj(NPY_FLOAT,age_univ_arr_Dims,age_univ_arr_Rank,capi_age_univ_arr_intent,Py_None);
  if (capi_age_univ_arr_tmp == NULL) {
    if (!PyErr_Occurred())
      PyErr_SetString(cnt_tree_error,"failed in converting hidden `age_univ_arr' of cnt_tree.load_tree to C/Fortran array" );
  } else {
    age_univ_arr = (float *)(PyArray_DATA(capi_age_univ_arr_tmp));

  /* Processing variable sonid */
  sonid_Dims[0]=n_all_sons;
  capi_sonid_intent |= F2PY_INTENT_OUT|F2PY_INTENT_HIDE;
  capi_sonid_tmp = array_from_pyobj(NPY_INT,sonid_Dims,sonid_Rank,capi_sonid_intent,Py_None);
  if (capi_sonid_tmp == NULL) {
    if (!PyErr_Occurred())
      PyErr_SetString(cnt_tree_error,"failed in converting hidden `sonid' of cnt_tree.load_tree to C/Fortran array" );
  } else {
    sonid = (int *)(PyArray_DATA(capi_sonid_tmp));

  /* Processing variable omega_t_arr */
  omega_t_arr_Dims[0]=nsteps;
  capi_omega_t_arr_intent |= F2PY_INTENT_OUT|F2PY_INTENT_HIDE;
  capi_omega_t_arr_tmp = array_from_pyobj(NPY_FLOAT,omega_t_arr_Dims,omega_t_arr_Rank,capi_omega_t_arr_intent,Py_None);
  if (capi_omega_t_arr_tmp == NULL) {
    if (!PyErr_Occurred())
      PyErr_SetString(cnt_tree_error,"failed in converting hidden `omega_t_arr' of cnt_tree.load_tree to C/Fortran array" );
  } else {
    omega_t_arr = (float *)(PyArray_DATA(capi_omega_t_arr_tmp));

  /* Processing variable fatherid */
  fatherid_Dims[0]=n_all_fathers;
  capi_fatherid_intent |= F2PY_INTENT_OUT|F2PY_INTENT_HIDE;
  capi_fatherid_tmp = array_from_pyobj(NPY_INT,fatherid_Dims,fatherid_Rank,capi_fatherid_intent,Py_None);
  if (capi_fatherid_tmp == NULL) {
    if (!PyErr_Occurred())
      PyErr_SetString(cnt_tree_error,"failed in converting hidden `fatherid' of cnt_tree.load_tree to C/Fortran array" );
  } else {
    fatherid = (int *)(PyArray_DATA(capi_fatherid_tmp));

  /* Processing variable aexp_arr */
  aexp_arr_Dims[0]=nsteps;
  capi_aexp_arr_intent |= F2PY_INTENT_OUT|F2PY_INTENT_HIDE;
  capi_aexp_arr_tmp = array_from_pyobj(NPY_FLOAT,aexp_arr_Dims,aexp_arr_Rank,capi_aexp_arr_intent,Py_None);
  if (capi_aexp_arr_tmp == NULL) {
    if (!PyErr_Occurred())
      PyErr_SetString(cnt_tree_error,"failed in converting hidden `aexp_arr' of cnt_tree.load_tree to C/Fortran array" );
  } else {
    aexp_arr = (float *)(PyArray_DATA(capi_aexp_arr_tmp));

  /* Processing variable fathermass */
  fathermass_Dims[0]=n_all_fathers;
  capi_fathermass_intent |= F2PY_INTENT_OUT|F2PY_INTENT_HIDE;
  capi_fathermass_tmp = array_from_pyobj(NPY_FLOAT,fathermass_Dims,fathermass_Rank,capi_fathermass_intent,Py_None);
  if (capi_fathermass_tmp == NULL) {
    if (!PyErr_Occurred())
      PyErr_SetString(cnt_tree_error,"failed in converting hidden `fathermass' of cnt_tree.load_tree to C/Fortran array" );
  } else {
    fathermass = (float *)(PyArray_DATA(capi_fathermass_tmp));

  /* Processing variable i_arr */
  i_arr_Dims[0]=n_halos_all,i_arr_Dims[1]=14;
  capi_i_arr_intent |= F2PY_INTENT_OUT|F2PY_INTENT_HIDE;
  capi_i_arr_tmp = array_from_pyobj(NPY_INT,i_arr_Dims,i_arr_Rank,capi_i_arr_intent,Py_None);
  if (capi_i_arr_tmp == NULL) {
    if (!PyErr_Occurred())
      PyErr_SetString(cnt_tree_error,"failed in converting hidden `i_arr' of cnt_tree.load_tree to C/Fortran array" );
  } else {
    i_arr = (int *)(PyArray_DATA(capi_i_arr_tmp));

  /* Processing variable fatheridx */
  fatheridx_Dims[0]=n_all_fathers;
  capi_fatheridx_intent |= F2PY_INTENT_OUT|F2PY_INTENT_HIDE;
  capi_fatheridx_tmp = array_from_pyobj(NPY_INT,fatheridx_Dims,fatheridx_Rank,capi_fatheridx_intent,Py_None);
  if (capi_fatheridx_tmp == NULL) {
    if (!PyErr_Occurred())
      PyErr_SetString(cnt_tree_error,"failed in converting hidden `fatheridx' of cnt_tree.load_tree to C/Fortran array" );
  } else {
    fatheridx = (int *)(PyArray_DATA(capi_fatheridx_tmp));

  /* Processing variable f_arr */
  f_arr_Dims[0]=n_halos_all,f_arr_Dims[1]=25;
  capi_f_arr_intent |= F2PY_INTENT_OUT|F2PY_INTENT_HIDE;
  capi_f_arr_tmp = array_from_pyobj(NPY_FLOAT,f_arr_Dims,f_arr_Rank,capi_f_arr_intent,Py_None);
  if (capi_f_arr_tmp == NULL) {
    if (!PyErr_Occurred())
      PyErr_SetString(cnt_tree_error,"failed in converting hidden `f_arr' of cnt_tree.load_tree to C/Fortran array" );
  } else {
...
/*end of pyobjfrom*/
    CFUNCSMESS("Building return value.\n");
    capi_buildvalue = Py_BuildValue("NNNNNNNNN",capi_fatherid_tmp,capi_fatheridx_tmp,capi_sonid_tmp,capi_fathermass_tmp,capi_i_arr_tmp,capi_f_arr_tmp,capi_aexp_arr_tmp,capi_omega_t_arr_tmp,capi_age_univ_arr_tmp);
/*closepyobjfrom*/
/*end of closepyobjfrom*/
    } /*if (f2py_success) after callfortranroutine*/
/*cleanupfrompyobj*/
