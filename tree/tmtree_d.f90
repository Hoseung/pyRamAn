subroutine count_tree(fn, n_halos_all, flist_index, slist_index, nsteps, big_run)
    ...
    real(KIND=4), dimension(:), allocatable::age_univ
    integer, INTENT(OUT)::flist_index, slist_index, n_halos_all, nsteps

    integer(KIND=4)::i,j, nhals_now, id
    ...
    ! REAL must be in double. (treebricks are in double!)
    read(1)!aexp(1:nsteps)
    read(1)!omegat(1:nsteps)
    read(1)!age_univ(1:nsteps)
    flist_index=0
    slist_index=0
    ...
    do i=1,nsteps
        ...
        do j=1,nhals_now
            ...
            read(1)!BushID
            read(1)!st
            read(1)!h%level,h%hosthalo,h%hostsub,h%nbsub,h%nextsub
            read(1)!h%m                                           !3
            read(1)!h%macc
            read(1)!h%p%x,h%p%y,h%p%z
            read(1)!h%v%x,h%v%y,h%v%z                             !6
            read(1)!h%L%x,h%L%y,h%L%z                             !7
            read(1)!h%r, h%sh%a, h%sh%b, h%sh%c                   !8
            read(1)!h%ek,h%ep,h%et                                !9
            read(1)!h%spin                                        !10
            read(1)nfathers
